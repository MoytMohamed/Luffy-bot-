        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                     this.reply(m.chat, `*لــيس لــديــك جـواهــر لـاسـخـدام هــذا الــامـر*\n*${usedPrefix}لجواهر*\n*أســتـخـدم هـذا الــامر لـلـحصـول عـلـى جـواهــر*`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`*لــقد أســتـخدمـت ${+m.diamond} 💎*`)
                }
                break
            }
        }
...
*↻╍╍━┛👑┗━╍╍↺*
*⟐╎ملحوظه هامة*
*نرجو منك سيدي الاوتاكو سيدتي الاوتاكو الإلتزام بالقوانين وقرائتها واذا واجهت اي مشكله توجه الى احد المشرفين او المطور وسوف نساعدك👨🏻‍💻🫱🏻‍🫲🏻.*
*↻╍╍━┛👑┗━╍╍↺*
*⨀⇠ تــوقــيــع ⇣*  
*『👑┇𝙻𝚄𝙵𝙵𝚈┇🤖┇𝙱𝙾𝚃┇👑』*`
                  ...
                  let secondText = `*نورت فــي افـضـل جـروب @user*`;
          
                  let welcomeApiUrl = await this.profilePictureUrl(user, 'image');;
 ...
 *『 https://wa.me/+967714075210 』*
*↻╍╍━┛👑┗━╍╍↺*
*⏣↵ شـكـراً لـك كـنـت خـيـر أنسـان عـرفـنـاه💞*
*↻╍╍━┛👑┗━╍╍↺*
*⨀⇠ تــوقــيــع ⇣*  
*『👑┇𝙻𝚄𝙵𝙵𝚈┇🤖┇𝙱𝙾𝚃┇👑』*`
                    .replace('@user', '@' + user.split('@')[0]);
                  ...
                  let nthMember = groupMetadata.participants.length;
                  let secondText = `*الــلي بـاعـنا خـسر دلـعـنا, @user*`;
          
                  let leaveApiUrl = await this.profilePictureUrl(user, 'image');;
          
                  try {
                    let leaveResponse = await fetch(leaveApiUrl);
                    let leaveBuffer = await leaveResponse.buffer();
          
                    this.sendFile(id, leaveApiUrl, 'leave.png', text, null, false, { mentions: [user] });
                  } catch (error) {
